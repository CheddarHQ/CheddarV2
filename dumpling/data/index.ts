export default [
  {
    id: 1,
    img: require('../assets/wobbleFIve.gif'),
    description: 'Talk & trade crypto - your decentralized chat room!',
  },
  {
    id: 2,
    img: require('../assets/wobbleTwo.png'),
    description: 'Join the convo – share tips and ride the crypto tide!',
  },
  {
    id: 3,
    img: require('../assets/wobbleThree.png'),
    description: 'Chat and swap – your next big opportunity is just a blink away!',
  },
  {
    id: 4,
    img: require('../assets/wobbleFour.png'),
    description: 'From GM to GMI – connect, swap, and stack SOL',
  },
];
